...
while read -r path; do
  ...
  if [ -d "$path" ]; then
    ...
    # Skip root directories like "content" or "static" or `.devfile`
    if [[ "$path" == "content" || "$path" == "static" || "$path" == ".devfile" ]]; then
      continue
    fi

    # For files located at the root directory, '.' is its path
    if [[ "$path" == "." ]]; then
      continue
    fi
...
EOF
    ...
    # Skip devfiles in the root and .devfile directory
    if [[ "$filename" == ".devfile.yaml" || "$filename" == ".devfile.yml" || "$path" == .devfile/* ]]; then
      continue
    fi
...
MISSING_FILE_ENTRY=""
while read ENTRY; do
  # Check if entry contains a wildcard
  if [[ "$ENTRY" == *"*"* ]]; then
    # Extract the part before the wildcard
    PREFIX="${ENTRY%%\**}"
    # Check if at least one file or folder matches the prefix
    if ! ls -d ${PREFIX}* >/dev/null 2>&1; then
      ERROR_FOUND=true
...
[
  {
    ...
    "description": "The wildcard entry \`$ENTRY\` is listed in CODEOWNERS but no files or folders match the pattern.  Please remove this CODEOWNER entry or add matching files",
    "severity": "major",
    "fingerprint": "$fingerprint",
    "location": {
...
EOF
    ...
    # No wildcard, check for exact match
    if ! [ -e $ENTRY ]; then
      ERROR_FOUND=true
      MISSING_FILE_ENTRY="$MISSING_FILE_ENTRY- $ENTRY\n"
      fingerprint=$(sha256sum .gitlab/CODEOWNERS)
...
while read LINE; do
  if ! echo $LINE | grep -qE "@gitlab-com/content-sites|@gitlab-com/egroup|@@owner"; then
    ERROR_FOUND=true
...
[
  {
    ...
    "description": "The entry for \`$FILE\` doesn't have the handbook and egroup groups (or @@owner) attached to it.  Please add them.",
    "severity": "major",
    "fingerprint": "$fingerprint",
    "location": {
      "path": ".gitlab/CODEOWNERS",
