...
## Paved Path to Production Readiness

For teams introducing new infrastructure components that cannot use [Runway](https://docs.runway.gitlab.com/),
the [Component Ownership Model](component-ownership-model.md) provides an opinionated paved path that significantly accelerates the Production Readiness process.
The Component Ownership Model is not an alternative to the Production Readiness process, but offers a faster smoother means to achieve Production Readiness.
...
The Component Ownership Model is particularly valuable for components requiring Self-Managed deployment compatibility or advanced stateful Kubernetes features not yet supported by Runway.
By following this paved path, teams can expect significantly reduced review times and smoother progression through the Production Readiness stages.

Teams considering this approach should review the [Component Ownership Model documentation](component-ownership-model.md) to understand the process,
requirements, and benefits of this accelerated path to production.
