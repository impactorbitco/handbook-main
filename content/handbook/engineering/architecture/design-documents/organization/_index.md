...
- Create an Organization (not intended for GA)
...
- Navigate between Organizations and preserve scope via Organization Context

Team members can opt out of the experience in case of emergency.

##### Organization Context and Isolation

Define Organization Context for web requests to a moderate degree.
Specifically this will apply to Routable models, GraphQL queries, and some Organization scoped routes.

We will continue to perform [database table sharding](https://gitlab.com/groups/gitlab-org/-/epics/11670)
to link tables back to an Organization where possible.
...
#### FY26 Q3 - Minimum fully isolated Organization

The target is to have a fully isolated Organization on the Legacy Cell.

To make this target feasible within a quarter the Organization will have the smallest Organization footprint possible.
This means the Organization will have a single user, who has been through the sign up process, and never returned to their account.

In order to get data off the Legacy Cell and onto the ProtoCell we must:

1. Deem a top-level group as being in an isolated state.
2. Enforce isolation on that top-level group to maintain state integrity.
    1. Unsupported Organization actions will invalidate the isolation state.
3. Promote that top-level group to an Organization.
    1. Create an Organization.
    2. Transfer the top-level group to the new Organization.
4. Enforce isolation on that Organization.

The following secondary targets will also be in place:

1. Organization Management: Port selected admin pages to Organization level (e.g. users)
2. Organization scoped paths: All necessary paths that don't define an Organization Context within the path, params, or headers will be modified to do so.
3. Database sharding: We will continue to shard database tables.
