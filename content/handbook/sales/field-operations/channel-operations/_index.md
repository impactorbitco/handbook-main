...
When a deal is being transacted via a Partner, a GitLab quote is **required**. At a minimum, a [draft proposal](/handbook/sales/field-operations/sales-operations/deal-desk/#how-to-create-a-draft-proposal) needs to be created and provided to the Partner and the partner must include the order form number on their Purchase Order. Quotes provided to partners that are not in SFDC are not permitted within any GEO or Sales Segment. This includes providing product and discounted pricing quote details in an email.

#### Partner Quoting Responsibilities

- Ecosystem Specialists are responsible for quoting Open and Select Partner opportunities in APJ (including AWS CPPO or GCP MCPO).
- Field Sales teams are responbisble for all other partner quotes including AMER, EMEA, and PubSec regions, and direct (non-CPPO/MCPO AWS and GCP, or IBM OEM transactions
- [PO remittance](/handbook/sales/field-operations/order-processing/#po-remittance) and customer communications tasks are owned by field sales teams.
...
Full Rules of Engagement for specific quote requst types can be found [here](https://gitlab.highspot.com/items/68b73dad5182d2d8fe342a7b?lfrm=rhp.0#1).
...
Distributors work directly with GitLab Sellers and/or Ecosysetm Specialists on the quoting and ordering process. GitLab Sellers can leverage the [partner quoting resources](/handbook/sales/field-operations/channel-operations/#partner-quoting-overview-and-resources), [distributor contact information](/handbook/sales/field-operations/channel-operations/#distributor-requirements-and-coverage-by-geo-and-market), and [GitLab Sales FAQ - Selling with Partners](/handbook/sales/field-operations/channel-operations/sales-faq/) to aid in the quote and order process.

If a reseller requests a quote directly from a distributor via the aliases above, non-PubSec distributors may submit a quote request through the partner portal or work with their account executive. Distributors may contact `partnersupport@gitlab.com` if they are unsure of who their account executive is. Partner Support will forward the request to the GitLab Sales Rep that owns the customer account/opportunity for Sales to work the opportunity with the distributor, including creating and providing the quote (if applicable).
...
- **DR - Partner**: The reseller partner that 'submitted' the Partner Sourced Deal Registration that the GitLab subsequently 'approved' in the system
...
- **Referral**: A link to the AWS/ACE co-sell registration record
...
**The SLA for GitLab to communicate with partners on a Partner Sourced Deal Registration is two business days. There must be contact with the registering partner within two business days, whether it be initial outreach to discuss the registration, a request for more information, approval, or rejection.** Registrations will be auto-approved after 10 days of submission if not already actioned.
...
A Service Attached Registration is classified as Closed Won and is rolled into the Clari forecast when all of the following criteria are met:
...
The Most Likely category captures opportunities where Ecosystem Directors have completed forecast entries. This rollup encompasses both Closed Won opportunities and active deals that meet the following requirements:

- `Close Date` falls within the selected quarter
...
- GitLab employees can access the [partner discount guidance here](https://gitlab.highspot.com/items/672950b937a5d0282b99cb96?lfrm=shp.0#1)
...
Price Files and an Ecosystem Discount Calculator for internal use can be found [in this folder](https://drive.google.com/drive/folders/1L-wxZmmIOA4DiOX7vZnOpBsiyBaxVr6S).
...
Our Cloud and Resale partner reps often request joint account mapping with their ESM to identify shared accounts in their territories. The following steps can be used to request an account mapping data pull.

1. ESM or Partner completes the [Account Mapping Template](https://docs.google.com/spreadsheets/d/1DrKmck-8B0hlXFerblB99odU6z9VBUewh3NBPPdTooc/edit?gid=0#gid=0) and sends it to the GitLab ESM.  Account Name and Domain (Columns A-B) are required for the account mapping tool to render results.

2. ESM sends the account mapping template to #global-ecosystem-programs-ops to request a partner mapping data run.

3. Ecosystem Specialist runs the account mapping template through the [Confidential] [Internal DI Only] Account Mapping 2026 sheet. This doc is only visible to the ecosystem specialist team due to SAFE requirements.

4. Ecosystem Specialist will copy the results onto two docs:

   - A standalone g-sheet doc that can be shared with the hyperscaler account rep.
   - The master [INTERNAL ONLY] Account Mapping Results sheet. This is an internal-only view of all account mapping requests. A new tab will be created for each request with the naming convention MM/DD/YY - [Requestor].

5. Ecosystem Specialists will clear the Account Mapping Template and Account Mapping sheet so they are ready for the next user.

6. ESM will share the results with their account rep.
...
To recognize and properly compensate these transactions, please ensure the `CPPO or DSOR Partner` field = "Amazon Web Services" on the opportunity record. Please chatter David La Tour or Pilar Meija on your DSOR opportunity so they can review the opportunity and update the `CPPO or DSOR Partner` field.
...
- For more information on our AWS CPPO program and process, please reference the following [program guide](https://docs.google.com/document/d/11P1Iva6ksIuw-UrxCvq5G5i2k0a-cplxAsCKi4msjpY/edit) and [process guide](https://docs.google.com/document/d/1kYGi4Nj7m0Ej_YlZwUQUasLtamJhW2vRv8-QyTTXh18/edit?tab=t.0#heading=h.wkx7iaqr4vzy).
- For more information on our GCP MCPO program and process, please reference the following [program and process guide.](https://docs.google.com/document/d/1S6bg1d6CobRVqqmJ-d9UT7ohZqGjgFb08v3-wjyiFbI/edit?tab=t.0)
...
AWS registrations can be received, submitted, and managed directly through your SFDC opportunity using the Tackle Cosell integration. Instructions can be found in the [Tackle Co-Sell | How To Guide](https://docs.google.com/document/d/1LFU0jQHbC0nPxxycYI7MGLvwirsj7o-weGQVXruS9T4/edit?tab=t.0) .
