...
Check for existing requests from the user. Should an existing request be found close the current issue as duplicate, see [Duplicate Form Submissions](#duplicate-form-submissions). No further action is required.
...
- **If verification passes**:
  - Request a peer review of your verification decision in Slack (`#support_gitlab-com`). Once peer-reviewed:
    - If the account is **active**, proceed to [**Step 3:** Create Meta Issue](#step-3-create-meta-issue).
    - If the account is **blocked/banned**, proceed to [**Step 2.5:** Verification Passed - Unblock User](#step-25-verification-passed---unblock-user).

- **If verification fails**:
  - If the user is **not** the sole owner of the groups and projects in their account, proceed to [Verification Failed](#verification-failed).
  - If the user **is** the sole owner of the groups and projects on their account, proceed to [Verification Failed (Sole Owner Exception)](#verification-failed---sole-owner-exception).
...
A user can additionally request that their GitLab forum account be deleted if the request comes from the Personal Data Request form. We can only delete users from the GitLab forum and have no control over the Discourse platform. To delete a forum user:
...
1. Apply the label `~deletion request::complied` to the issue.
