...
In cases where you are unable to find any log entries in Cloudflare or Kibana for an IP block, it may be required to reach out to our Production Engineering team. We can [request additional help from the Production Engineering team](/handbook/engineering/infrastructure/rate-limiting/troubleshooting/#requesting-further-assistance) in scenarios like this.

## False positive IP geolocation block

In the case where a user is being blocked from accessing GitLab.com and it is suspected to be a false positive geolocation block we can [request additional help from the Production Engineering team](/handbook/engineering/infrastructure/rate-limiting/troubleshooting/#requesting-further-assistance).
