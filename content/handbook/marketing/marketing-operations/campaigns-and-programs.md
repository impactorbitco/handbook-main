...
| Waitlisted | Holding state if registration is full will be moved to `Registered` if space opens |  | No |
| Registered | Registered for event |  | No |
...
- `Employee Bucket` (Employee count): Values [here](/handbook/marketing/marketing-operations/automated-list-import/#data-cleaning-instructions). These values must be written exactly as shown (using commas and correct spacing).
...
- **Invalid**: The email address was deemed invalid by Marketo. Either our emails are being blocked, the email address doesn't exist or the domain was identified as a disposble email address. We cannot accept or use the lead because Marketo cannot determine if the email address is real
...
- When a lead is sent into our system by Integrate, they are timestamped by [the Processing for Initial Integrate Closed Loop](https://experience.adobe.com/#/@gitlab/so:194-VVC-221/marketo-engage/classic/SC64759A1ZN19) smart campaign. If a lead is rejected by the closed loop feedback logic and is eventually returned to us after Integrate corrects the lead's info, the [Processing for Repeat Integrate Closed Loop](https://experience.adobe.com/#/@gitlab/so:194-VVC-221/marketo-engage/classic/SC64760A1ZN19) smart campaign resets some lead statuses and their reasons. This way the leads do not re-trip the closed loop logic with outdated info.
