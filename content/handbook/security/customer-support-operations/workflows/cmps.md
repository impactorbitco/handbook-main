...
This feature is soon to be deprecated. The information will remain here for historic purposes.
...
- The organization has a paid and active subscription
- The paid subscription must be for at least 500 seats
- The request must be initiated by the owner (sold-to) of the paid subscription

If any of those criteria are not met, you should reply to the ticket with the `Customer Support Operations::Not qualified for CMP` macro.

If they have met all criteria, you can proceed to the approval section below.

## Approval

As this is a beta feature slated for deprecation in the near future, we need to make sure the customer is aware of this and still wants to proceed.

To do this, you should reply to the ticket with the `Customer Support Operations::CMP intro` macro.

If they have replied back confirming they still wish to proceed, you can move to the setup section below.
...
The below information details how to manually setup a contact management project. This should only ever be used in the event of a unfixable error from the application (which cannot be fixed in a reasonable amount of time). Consider contacting the Fullstack Engineer, Customer Support Operations or filing an incident before proceeding in doing this manually.
