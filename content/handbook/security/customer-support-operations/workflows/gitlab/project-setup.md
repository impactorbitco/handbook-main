...
We all must follow this process to ensure our projects are setup properly in a secured and maintainable way. If every you are questioning what to do, please reach out to your fellow team members.
...
The exact one you will use depends on what the project's purpose is. If you are ever unsure which to use, reach out to the other Customer Support Operations team members to determine which one is the ideal one to use.
...
1. Navigate to the correct group
   ...
   - If you are ever unsure which to use, reach out to the other Customer Support Operations team members to determine which one is the ideal one to use
1. Click `New project` at the top-right of the page
   - If you do not see that button, please reach out to a Fullstack Engineer, Customer Support Operations for assistance.
...
- Webhooks
  - None
    - NOTE: You might be adding some later if needed, but initially there should not be any
- Access tokens
  - None
    - NOTE: You might be adding some later if needed, but initially there should not be any
- Repository
...
- If this is a US citizen only project (which should be very rare), then add the group `gitlab-support-readiness/cust_support_ops_team_us_citizens` as `Developers`
- If this is something requiring critical level approval (which should be very rare), then add the group `gitlab-support-readiness/cust_support_ops_critical_access` as `Developers`
- For everything else, then add the group `gitlab-support-readiness/cust_support_ops_team` as `Developers`
